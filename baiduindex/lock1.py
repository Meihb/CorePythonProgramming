...
def test():
    ...
    cur.execute('SELECT * FROM `baidu_index`  WHERE  process_status = 0 ORDER BY id ASC limit 1 FOR UPDATE')
    id = cur.fetchall()[0]['id']
    # id = 1
    cur.execute('UPDATE  `baidu_index`  SET process_status = -1  WHERE  id=%s',(id))
